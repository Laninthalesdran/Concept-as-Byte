"""
Matched Comparison — Concept Byte Model (7.18M)
================================================
Same architecture, same Esperanto sentences, concept byte encoding.
Vocab: 256 (single-byte concepts)
d_model=384, n_layers=3, n_heads=8, d_ff=1536
...
DATA_FILE = os.path.join(SCRIPT_DIR, 'concept_byte_training.bin')
CHECKPOINT_DIR = os.path.join(SCRIPT_DIR, 'cb_checkpoints')
LOG_DIR = os.path.join(SCRIPT_DIR, 'cb_logs')
RESULTS_FILE = os.path.join(SCRIPT_DIR, 'results_concept_byte.json')
...
# ============================================================
# MODEL — 7.18M Concept Byte (vocab=256, d_ff=1536)
# ============================================================
...
class ConceptByteModel(nn.Module):
    def __init__(self):
        super().__init__()
...
# ============================================================
# DATASET — byte chains split on END (0x00)
# ============================================================

class ByteChainDataset(Dataset):
...
def train():
    log_file = os.path.join(LOG_DIR, 'training_concept_byte.log')
    logging.basicConfig(level=logging.INFO, format='%(asctime)s | %(message)s',
        handlers=[logging.FileHandler(log_file, encoding='utf-8', mode='w'),
                  logging.StreamHandler(sys.stdout)])
    logger = logging.getLogger('concept_byte')

    logger.info("=" * 60)
    logger.info("MATCHED COMPARISON — Concept Byte Model")
    logger.info(f"Vocab: {VOCAB_SIZE} | d={D_MODEL} L={N_LAYERS} h={N_HEADS} ff={D_FF}")
    logger.info("=" * 60)
    logger.info(f"Device: {DEVICE}")

    dataset = ByteChainDataset(DATA_FILE, SEQ_LEN)
    ...
    model = ConceptByteModel().to(DEVICE)
    ...
    logger.info("")
    logger.info("=" * 60)
    logger.info(f"CONCEPT BYTE — FINAL RESULTS")
    logger.info(f"Parameters: {param_count:,}")
